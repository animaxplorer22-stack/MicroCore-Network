#!/usr/bin/env python3
"""
MICROCORE (MCX) COMPLETE NODE - FINAL VERSION
50 MCX BLOCK REWARD | ALL FEATURES: PoMA+PoS, P2P, DNS seeds, DEX (Own pools + LI.FI/THORChain), Leaderboards

Usage:
  python3 node_full.py --genesis              (First node - creates blockchain)
  python3 node_full.py --peer IP:PORT         (Join existing network)
  python3 node_full.py --no-miner             (Disable embedded miner)
"""
...
# TOKENOMICS - 100 MILLION HARD CAP, 50 MCX BLOCK REWARD
...
VALIDATOR_SHARE = 0.70      # 70% to validators (10 people)
NODE_SHARE = 0.08           # 8% to nodes
UPTIME_SHARE = 0.05         # 5% to uptime pool
LP_SHARE = 0.05             # 5% to liquidity providers
BUYER_REWARDS_SHARE = 0.12  # 12% to buyer rewards pool (monthly leaderboard)
...
class DEXBridge:
    def __init__(self, network=None):
        ...
        self.own_pools = {
            "MCX/USDC": {"token_a": "MCX", "token_b": "USDC", "reserve_a": 0, "reserve_b": 0, "fee": SWAP_FEE_RATE, "lp_providers": {}, "total_lp_shares": 0},
            "MCX/BTC": {"token_a": "MCX", "token_b": "BTC", "reserve_a": 0, "reserve_b": 0, "fee": SWAP_FEE_RATE, "lp_providers": {}, "total_lp_shares": 0},
            "MCX/ETH": {"token_a": "MCX", "token_b": "ETH", "reserve_a": 0, "reserve_b": 0, "fee": SWAP_FEE_RATE, "lp_providers": {}, "total_lp_shares": 0},
            "MCX/SOL": {"token_a": "MCX", "token_b": "SOL", "reserve_a": 0, "reserve_b": 0, "fee": SWAP_FEE_RATE, "lp_providers": {}, "total_lp_shares": 0},
            "MCX/BNB": {"token_a": "MCX", "token_b": "BNB", "reserve_a": 0, "reserve_b": 0, "fee": SWAP_FEE_RATE, "lp_providers": {}, "total_lp_shares": 0}
        }
        
        self.external_swaps = []
    
    def connect(self) -> bool:
        print(f"[DEX] LI.FI API: {self.lifi_api_url}")
        print(f"[DEX] THORChain API: {self.thorchain_api_url}")
    ...
    def get_own_pool_quote(self, from_token: str, to_token: str, amount: float) -> dict:
        if from_token == "MCX":
            ...
            return {"success": True, "pool_type": "own", "pool_id": pool_id, "from_token": from_token, "to_token": to_token,
                    "amount_in": amount, "expected_output": output, "fee_mcx": fee_mcx, "route": f"Own pool: {pool_id}"}
        else:
    ...
    async def execute_own_pool_swap(self, user_wallet: str, from_token: str, to_token: str, amount: float, fee_mcx: int) -> dict:
        ...
        return {"success": True, "tx_hash": tx_hash, "pool_type": "own", "from_token": from_token, "to_token": to_token,
    ...
    async def add_own_pool_liquidity(self, user_wallet: str, pool_id: str, amount_a: float, amount_b: float) -> dict:
        ...
        return {"success": True, "pool_id": pool_id, "amount_a": amount_a, "amount_b": amount_b,
                "lp_shares": lp_shares, "total_lp_shares": pool["total_lp_shares"]}
    
    # ==================== OPTION B: LI.FI / THORCHAIN ====================
    async def get_lifi_quote(self, from_token: str, to_token: str, amount: float) -> dict:
        ...
        return {"success": True, "pool_type": "lifi", "from_token": from_token, "to_token": to_token,
                ...
                "route": f"{from_token} → {to_token} via LI.FI", "aggregator": "lifi"}
    ...
    async def get_thorchain_quote(self, from_token: str, to_token: str, amount: float) -> dict:
        ...
        return {"success": True, "pool_type": "thorchain", "from_token": from_token, "to_token": to_token,
                "amount_in": amount, "expected_output": expected_output, "fee_mcx": fee_mcx,
                "route": f"{from_token} → {to_token} via THORChain", "aggregator": "thorchain"}
    ...
    async def execute_swap(self, user_wallet: str, from_token: str, to_token: str, amount: float, fee_mcx: int) -> dict:
        ...
        tx_hash = hashlib.sha256(f"{user_wallet}{from_token}{to_token}{amount}{time.time()}".encode()).hexdigest()[:16]
        return {"success": True, "tx_hash": tx_hash, "pool_type": quote.get("pool_type", "external"),
                "from_token": from_token, "to_token": to_token, "amount_in": amount,
                "amount_out": quote.get("expected_output", 0), "fee_mcx": fee_mcx}
    
    async def get_supported_pools(self) -> List[dict]:
        pools = []
        for pool_id, pool in self.own_pools.items():
            pools.append({"type": "own", "pool_id": pool_id, "token_a": pool["token_a"], "token_b": pool["token_b"],
                         "reserve_a": pool["reserve_a"], "reserve_b": pool["reserve_b"], "fee": pool["fee"]})
...
class MicroCoreNetwork:
    ...
    def init_database(self):
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS transactions
                     (tx_hash TEXT PRIMARY KEY, from_wallet TEXT, to_wallet TEXT,
                      ...
                      signature TEXT, status TEXT, nonce INTEGER, type TEXT)''')
    ...
    def create_genesis_block(self):
        ...
        print(f"[GENESIS] Reward split: 70% validators | 8% nodes | 5% uptime | 5% LP | 12% buyer rewards")
        print("=" * 70 + "\n")
    ...
    def transfer(self, from_wallet: str, to_wallet: str, amount: int, fee: int = 1, signature: str = "", tx_type: str = "send") -> Optional[str]:
        ...
        # Track buyer stats for leaderboard
        if tx_type == "buy":
            c = self.conn.cursor()
            c.execute("INSERT INTO buyer_stats (wallet, username, monthly_bought, last_reset) VALUES (?, ?, ?, ?) ON CONFLICT(wallet) DO UPDATE SET monthly_bought = monthly_bought + ?",
                     (from_wallet, from_wallet, amount, time.time(), amount))
            self.conn.commit()
        ...
        c.execute("INSERT INTO transactions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                 (tx_hash, from_wallet, to_wallet, amount, fee, time.time(), -1, signature, "pending", 0, tx_type))
        self.conn.commit()
        return tx_hash
    ...
    def distribute_block_reward(self, block: Block):
        ...
        # NEW DISTRIBUTION: 70% validators, 8% nodes, 5% uptime, 5% LP, 12% buyer rewards
        validator_total = (reward * 70) // 100      # 70% of 50 = 35 MCX
        node_total = (reward * 8) // 100            # 8% of 50 = 4 MCX
        uptime_total = (reward * 5) // 100          # 5% of 50 = 2 MCX
        lp_total = (reward * 5) // 100              # 5% of 50 = 2 MCX
        buyer_total = (reward * 12) // 100          # 12% of 50 = 6 MCX
        
        validator_each = validator_total // max(len(block.validators), 1)  # 35 // 10 = 3 MCX each
    ...
    def distribute_buyer_rewards(self):
        """Distribute buyer rewards pool to top 10 buyers monthly"""
        ...
        for i, (wallet, username, _) in enumerate(top_buyers):
            ...
            c.execute("INSERT INTO transactions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                     (tx_hash, "buyer_rewards_pool", wallet, reward, 0, time.time(), -1, "", "confirmed", 0, "reward"))
            self.conn.commit()
...
class MicroCoreServer:
                ...
                if t == "register":
                    ...
                    if self.network.register_miner(vid, pubkey, username, wallet, stake, sig, ts):
                        ...
                        await websocket.send(json.dumps({
                            ...
                            "symbol": SYMBOL, "reward_split": "70/8/5/5/12"
                        }))
                    ...
                    amount = data["amount"]
                    if self.network.get_balance(data["wallet"]) < amount:
                        await websocket.send(json.dumps({"type": "buy_failed", "reason": "Insufficient balance"}))
                    else:
                        tx_hash = self.network.transfer(data["wallet"], "dex_pool", amount, 1, data.get("signature", ""), "buy")
                        if tx_hash:
                            await websocket.send(json.dumps({"type": "buy_confirmed", "tx_hash": tx_hash}))
                            # Update buyer stats
                            c = self.network.conn.cursor()
                            c.execute("INSERT INTO buyer_stats (wallet, username, monthly_bought, last_reset) VALUES (?, ?, ?, ?) ON CONFLICT(wallet) DO UPDATE SET monthly_bought = monthly_bought + ?",
                                     (data["wallet"], data.get("username", ""), amount, time.time(), amount))
                            self.network.conn.commit()
                
                elif t == "send":
                    to = data["to"]
                    amount = data["amount"]
                    tx_hash = self.network.transfer(data["from"], to, amount, 1, data.get("signature", ""), "send")
                    if tx_hash:
                    ...
                    amount = data["amount"]
                    if self.network.get_balance(data["wallet"]) < amount:
                        await websocket.send(json.dumps({"type": "stake_failed", "reason": "Insufficient balance"}))
                    else:
                        self.network.transfer(data["wallet"], "staking_pool", amount, 1, data.get("signature", ""), "stake")
                        # Update miner stake
                        for miner in self.network.miners.values():
                            if miner.wallet == data["wallet"]:
                                miner.stake += amount
                                miner.level = self.network.calculate_level(miner.stake)
                                break
                        await websocket.send(json.dumps({"type": "stake_confirmed", "amount": amount}))
                
                elif t == "unstake":
                    amount = data["amount"]
                    for miner in self.network.miners.values():
                        if miner.wallet == data["wallet"] and miner.stake >= amount:
                            miner.stake -= amount
                            miner.level = self.network.calculate_level(miner.stake)
                            self.network.transfer("staking_pool", data["wallet"], amount, 1, data.get("signature", ""), "unstake")
                            await websocket.send(json.dumps({"type": "unstake_confirmed", "amount": amount}))
                            break
                    else:
                        await websocket.send(json.dumps({"type": "unstake_failed", "reason": "Insufficient stake"}))
                    ...
                    c = self.network.conn.cursor()
                    c.execute("SELECT tx_hash, from_wallet, to_wallet, amount, fee, timestamp, status, type FROM transactions ORDER BY timestamp DESC LIMIT 20")
    ...
    async def run(self):
        ...
        async with websockets.serve(self.handle, NODE_HOST, NODE_PORT):
            ...
            print(f"[REWARD] 50 MCX per block | Split: 70% validators | 8% nodes | 5% uptime | 5% LP | 12% buyer rewards")
